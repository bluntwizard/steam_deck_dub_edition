...
const IMPORT_TRANSFORMS = [
  // Add TypeScript type imports
  {
    ...
    replacement: (match, componentName, componentPath) => {
      return `import { ${componentName} } from '../../components/${componentPath}';\nimport { ${componentName}Options } from '../../types/${componentPath.toLowerCase()}';\n`;
    }
  },
  // Transform normal imports to TypeScript syntax
  {
    regex: /require\(['"]([^'"]+)['"]\)/g,
    replacement: (match, modulePath) => `import * as ${path.basename(modulePath, path.extname(modulePath))} from '${modulePath}'`
  }
];
