...
/**
 * Initialize the layout system when the DOM is ready
 */
 ...
 */
function setupPageStructure(): void {
...
/**
 * CSS variable names to copy from root to SVG
 */
const SVG_CSS_VARIABLES = [
  '--color-main',
  '--color-primary',
  '--color-secondary',
  '--color-background',
 ...
 */
function handleSvgWithInternalStyling(): void {
  const svgContainer = document.querySelector('.header-svg') as HTMLObjectElement | null;
    ...
    svgContainer.addEventListener('load', function() {
      try {
        ...
        if (svgDoc && svgDoc.documentElement) {
          ...
          let styleText = ':root {\n';
          SVG_CSS_VARIABLES.forEach(varName => {
...
/**
 * Export the layout functions for use in other modules
 */
export {
  setupPageStructure,
  initSidebar,
