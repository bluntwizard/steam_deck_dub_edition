...
class ContentLoader {
     ...
     */
    async _loadContentInternal(contentFile, append) {
        try {
    ...
    initializeLoadedContent() {
        ...
        document.querySelectorAll('details.guide-section').forEach(details => {
            ...
            if (summary && !summary._initialized) {
                summary._initialized = true;
 ...
 */
function processContentLayout(contentElement) {
  ...
  listSections.forEach(listElement => {
    ...
    if (parentSection && !parentSection.classList.contains('section-features') && listElement.children.length >= 3) {
  ...
  // Rest of processContentLayout implementation
  // ...

  // Additional layout processing functions can be added here
}
