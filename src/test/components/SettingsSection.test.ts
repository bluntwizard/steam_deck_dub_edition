...
describe("SettingsSection", () => {
  ...
  describe("Initialization", () => {
    test("should initialize with default options when autoInit is true", () => {
      const settingsSection: SettingsSection = new SettingsSection();
  ...
  describe("DOM Structure", () => {
    test("should create a section with correct attributes and classes", () => {
      ...
      expect(
        settingsSection.sectionElement.getAttribute("aria-labelledby"),
    ...
    test("should create settings based on configuration", () => {
      ...
      const settingElements =
        settingsSection.sectionElement.querySelectorAll("[data-setting-id]");
      expect(settingElements.length).toBe(2);

      const toggleSetting = settingsSection.sectionElement.querySelector(
        '[data-setting-id="setting1"]',
      );
      expect(toggleSetting).not.toBeNull();
      expect(toggleSetting.querySelector("label").textContent).toBe(
        "Toggle Setting",
      );
      expect(
        toggleSetting.querySelector('input[type="checkbox"]'),
      ).not.toBeNull();

      const selectSetting = settingsSection.sectionElement.querySelector(
        '[data-setting-id="setting2"]',
      );
      expect(selectSetting).not.toBeNull();
      expect(selectSetting.querySelector("label").textContent).toBe(
        "Select Setting",
      );
  ...
  describe("Setting Controls", () => {
    test("should create toggle switch", () => {
      ...
      const toggle1 =
        settingsSection.sectionElement.querySelector("#setting-toggle1");
      ...
      const toggle2 =
        settingsSection.sectionElement.querySelector("#setting-toggle2");
    ...
    test("should create select dropdown", () => {
      ...
      const select =
        settingsSection.sectionElement.querySelector("#setting-select1");
    ...
    test("should create radio button group", () => {
      ...
      const radioButtons = settingsSection.sectionElement.querySelectorAll(
        'input[type="radio"]',
      );
      expect(radioButtons.length).toBe(3);

      // Check that the correct option is selected
      const selectedRadio = settingsSection.sectionElement.querySelector(
        'input[value="option2"]',
      );
    ...
    test("should create text input", () => {
      ...
      const input =
        settingsSection.sectionElement.querySelector("#setting-text1");
    ...
    test("should create slider", () => {
      ...
      const slider =
        settingsSection.sectionElement.querySelector("#setting-slider1");
      ...
      const valueDisplay =
        settingsSection.sectionElement.querySelector(".sliderValue");
    ...
    test("should create button", () => {
      ...
      const button =
        settingsSection.sectionElement.querySelector("#setting-button1");
  ...
  describe("Event Handling", () => {
    ...
    test("should dispatch custom event when setting is changed", () => {
      ...
      settingsSection.sectionElement.addEventListener(
        "setting-change",
        mockSectionListener,
      );
  ...
  describe("Public Methods", () => {
    test("updateSetting should update setting value", () => {
      ...
      expect(
        settingsSection.sectionElement.querySelector("#setting-select1").value,
      ).toBe("option2");

      // Update slider
      settingsSection.updateSetting("slider1", 75);
      expect(
        settingsSection.sectionElement.querySelector("#setting-slider1").value,
      ).toBe("75");
      expect(
        settingsSection.sectionElement.querySelector(".sliderValue")
          .textContent,
    ...
    test("setSettingDisabled should update setting disabled state", () => {
      ...
      const radioInputs = settingsSection.sectionElement.querySelectorAll(
        'input[name="setting-radio1"]',
      );
    ...
    test("addSetting should add a new setting to the section", () => {
      ...
      const toggleElement = settingsSection.sectionElement.querySelector(
        '[data-setting-id="newToggle"]',
      );
    ...
    test("removeSetting should remove a setting from the section", () => {
      ...
      expect(
        settingsSection.sectionElement.querySelector(
          '[data-setting-id="toggle1"]',
        ),
      ).not.toBeNull();
      expect(
        settingsSection.sectionElement.querySelector(
          '[data-setting-id="toggle2"]',
        ),
      ).not.toBeNull();
