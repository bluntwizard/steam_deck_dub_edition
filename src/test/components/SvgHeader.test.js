...
describe('SvgHeader', () => {
      ...
      if (tag === 'object') {
        ...
        Object.defineProperty(element, 'contentDocument', {
          get: jest.fn().mockReturnValue({
            documentElement: document.implementation.createHTMLDocument().documentElement,
            querySelector: jest.fn().mockImplementation((selector) => {
              return document.implementation.createHTMLDocument().querySelector(selector);
            })
          })
        });
      }
  ...
  describe('Initialization', () => {
    ...
    test('should not initialize when autoInit is false', () => {
      const svgHeader = new SvgHeader({ autoInit: false });
  ...
  describe('Configuration', () => {
    ...
    test('should override options when provided', () => {
      ...
      const svgHeader = new SvgHeader(customOptions);
  ...
  describe('DOM Structure', () => {
    test('should create a header container with correct classes', () => {
      ...
      expect(svgHeader.headerContainer.classList.contains('header-container')).toBe(true);
      expect(svgHeader.headerContainer.classList.contains('svg-loading')).toBe(true);
    });
  ...
  describe('Event Handling', () => {
    ...
    test('should handle SVG load event correctly', () => {
      ...
      const mockSvgDoc = {
        ...
        querySelector: jest.fn().mockReturnValue(document.createElement('head'))
      };
      ...
      expect(svgHeader.headerContainer.classList.contains('svg-loading')).toBe(false);
      expect(svgHeader.headerContainer.classList.contains('svg-loaded')).toBe(true);
    });
    ...
    test('should handle SVG error event correctly', () => {
      ...
      // Check if classes were updated correctly
      expect(svgHeader.headerContainer.classList.contains('svg-loading')).toBe(false);
      expect(svgHeader.headerContainer.classList.contains('svg-error')).toBe(true);
    });
  });
