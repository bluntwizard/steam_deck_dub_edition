...
describe("KeyboardShortcuts Component", () => {
  ...
  test("should initialize with default shortcuts", () => {
    ...
    expect(keyboardShortcuts.shortcuts[0]).toEqual({
      keys: ["/"],
      description: "Focus search box",
    });
    expect(keyboardShortcuts.shortcuts[1]).toEqual({
      keys: ["?"],
  ...
  test("should create help button during initialization", () => {
    ...
    const createButtonCalls = document.createElement.mock.calls.filter(
      (call) => call[0] === "button",
    );
  ...
  test("should create dialog when show is called", () => {
    ...
    expect(document.addEventListener).toHaveBeenCalledWith(
      "click",
      keyboardShortcuts.handleOutsideClick,
    );
  });
  ...
  test("should not hide dialog for clicks on help button", () => {
    ...
    const mockEvent = {
      target: {
        classList: {
          contains: jest.fn().mockImplementation((cls) => cls === "helpButton"),
        },
      },
    };
  ...
  test("should handle ? key press to toggle shortcuts", () => {
    ...
    const isInputFocusedSpy = jest
      .spyOn(keyboardShortcuts, "isInputFocused")
