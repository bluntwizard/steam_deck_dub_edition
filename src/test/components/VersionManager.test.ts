...
describe("VersionManager", () => {
  ...
  describe("Initialization", () => {
    ...
    test("should override default options with provided options", () => {
      ...
      expect(versionManager.options.checkInterval).toBe(
        customOptions.checkInterval,
      );
    });
  ...
  describe("UI Components", () => {
    ...
    test("should create changelog dialog when showChangelog is called", async () => {
      ...
      const firstVersionTitle =
        versionEntries[0].querySelector(".versionTitle");
      expect(firstVersionTitle.textContent).toContain("1.3.0");

      const firstVersionChanges =
        versionEntries[0].querySelectorAll(".changesList li");
    ...
    test("should highlight current version in changelog", async () => {
      ...
      const versionEntries = document.querySelectorAll(".versionEntry");
      const currentVersionEntry = Array.from(versionEntries).find((entry) =>
        entry
          .querySelector(".versionTitle")
          .textContent.includes(currentVersion),
      );
