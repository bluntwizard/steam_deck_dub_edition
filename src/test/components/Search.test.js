...
describe('Search', () => {
  ...
  test('should perform search when button is clicked', () => {
    ...
    expect(mockSearchService.addToRecentSearches).toHaveBeenCalledWith('test query');
  });
  ...
  test('should perform search on Enter key press', () => {
    ...
    expect(mockSearchService.addToRecentSearches).toHaveBeenCalledWith('enter key test');
  });
  ...
  test('should display search results', () => {
    ...
    expect(resultItems[0].querySelector('a').getAttribute('href')).toBe('/page1');
  });
