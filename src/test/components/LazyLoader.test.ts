...
import type { LazyLoaderOptions } from "../../types/lazyloader";
// Mock fetch
...
describe("LazyLoader Component", () => {
  ...
  beforeEach(() => {
    ...
    global.fetch.mockImplementation(() =>
      Promise.resolve({
        ok: true,
        text: () =>
          Promise.resolve('<div class="test-content">Test Content</div>'),
      }),
    );
  ...
  test("initializes with default options", () => {
    const defaultLoader: LazyLoader = new LazyLoader();
  ...
  test("initializes with custom options", () => {
    const customLoader: LazyLoader = new LazyLoader({
  ...
  test("auto initializes when autoInit is true", () => {
    const autoLoader: LazyLoader = new LazyLoader({ autoInit: true });
  ...
  test("buildContentUrl formats URLs correctly", () => {
    // Absolute URL
    expect(lazyLoader.buildContentUrl("https://example.com/content")).toBe(
      "https://example.com/content",
    );

    // Relative URL without extension
    expect(lazyLoader.buildContentUrl("section1")).toBe(
      "./test-content/section1.html",
    );

    // Relative URL with extension
    expect(lazyLoader.buildContentUrl("section2.html")).toBe(
      "./test-content/section2.html",
    );
  });
  ...
  test("showLoading displays loading indicator", () => {
    ...
    expect(section.getAttribute("data-original-content")).toBe(
      "Original content",
    );
